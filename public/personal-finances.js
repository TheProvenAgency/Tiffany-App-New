...
(function(){
var C={blue:'#3563a8',green:'#2e7d54',gold:'#b98a2f',red:'#b3372f',teal:'#2f8f8a',purple:'#6b5bd0',slate:'#6b6256',gray:'#b9b3a8'};
...
function initLM(){
  ...
  if(typeof window.showView==='function'&&!window.__lmWrap){
    ...
    window.showView=function(id){
      if(id==='personal'){
        var vs=document.querySelectorAll('[id^="view-"]');for(var i=0;i<vs.length;i++)vs[i].style.display='none';
        ...
        var nb=document.getElementById('lmNavBtn');if(nb)nb.classList.add('active');
        var pt=document.getElementById('pageTitle');if(pt)pt.textContent='Personal Finances';
        renderPersonal();
      } else { var v=document.getElementById('view-personal');if(v)v.style.display='none';var nb=document.getElementById('lmNavBtn');if(nb)nb.classList.remove('active');_sv(id); }
    };
  }
}
