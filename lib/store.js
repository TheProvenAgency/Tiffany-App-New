...
// A person can mark a client "Affiliate" or "Needs affiliate" by hand from
// the client drawer, overriding the computed email/name match against the
// synced MyFreeScoreNow member list -- e.g. Tiffany knows someone signed up
// under her link with a different email than the one on file. Cleared
// (deleted from the map) to go back to the computed match.
// map: clientId -> 'affiliate' | 'needs'
...
function setAffiliateOverride(clientId, value) {
  ...
  if (value === 'affiliate' || value === 'needs') all[clientId] = value;
