...
async function mirrorUsers(users) {
  if (!db.isEnabled() || !Array.isArray(users)) return;
  for (const u of users) {
    if (!u || !u.id || !u.username) continue;
    try {
      await db.query(
        `insert into users (app_user_id, username, name, role, capabilities, disabled, sso_only)
...
async function hydrateDashboardLayoutsFromPostgres() {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      if (!key) continue;
      out[key] = { order: r.widget_order || [], sizes: r.widget_sizes || {} };
    }
    if (!Object.keys(out).length) return;
