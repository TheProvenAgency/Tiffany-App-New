...
async function hydrateDashboardLayoutsFromPostgres() {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      const hidden = (r.widget_sizes && Array.isArray(r.widget_sizes.hidden)) ? r.widget_sizes.hidden : [];
      out[key] = { nodes, hidden };
    }
    if (!Object.keys(out).length) return;
...
// A client's "you're welcome" is not somebody waiting on an answer, but the
// queue counts anyone who spoke last -- so the team can close a thread out.
// The dismissal is pinned to THAT message (last_msg_at): if the client sends
// anything new, the timestamps no longer match and the thread returns to the
// queue on its own. Nothing to remember, nothing that can silently hide a
// real question.
function getReplyDismissals() { return read('replyDismissals', {}); }
function setReplyDismissal(convId, lastAt, who) {
  const all = getReplyDismissals();
  if (lastAt === null) {
    delete all[convId];
    write('replyDismissals', all);
    if (db.isEnabled()) db.query('delete from reply_dismissals where conv_id = $1', [String(convId)]).catch(e => console.error('Dismissal delete mirror failed:', e.message));
    return null;
  }
  const entry = { lastAt: String(lastAt || ''), who: who || '', at: new Date().toISOString() };
  all[convId] = entry;
  write('replyDismissals', all);
  if (db.isEnabled()) {
    db.query(
      `insert into reply_dismissals (conv_id, last_msg_at, dismissed_by, dismissed_at) values ($1,$2,$3,now())
       on conflict (conv_id) do update set last_msg_at = excluded.last_msg_at, dismissed_by = excluded.dismissed_by, dismissed_at = now()`,
      [String(convId), entry.lastAt, entry.who]
    ).catch(e => console.error('Dismissal mirror failed (still saved locally):', e.message));
  }
  return entry;
}
async function hydrateReplyDismissalsFromPostgres() {
  if (!db.isEnabled()) return;
  if (Object.keys(getReplyDismissals()).length) return;
  try {
    const { rows } = await db.query('select conv_id, last_msg_at, dismissed_by, dismissed_at from reply_dismissals');
    if (!rows.length) return;
    const all = {};
    for (const r of rows) all[r.conv_id] = {
      lastAt: r.last_msg_at, who: r.dismissed_by,
      at: r.dismissed_at instanceof Date ? r.dismissed_at.toISOString() : String(r.dismissed_at)
    };
    write('replyDismissals', all);
    console.log(`Restored ${rows.length} reply dismissals from Postgres`);
  } catch (e) { console.error('Dismissal hydration failed:', e.message); }
}

// ---- client audits (Tiffany's file-by-file audit of the whole book) ----
// One entry per client, keyed by the roster id: which outcome the audit
// reached, who reached it, when. A client with no entry has not been
// audited -- which is exactly how clients that arrive AFTER the audit
// show up as needing one, with no extra bookkeeping.
// outcome: 'graduated' (credit fully finished -- upsell material)
//        | 'completed' (paid rounds fulfilled, may still need work)
//        | 'free_round' (ran out of rounds -- doing one free to re-engage)
//        | 'in_progress' (still in repair, keep working)
function getClientAudits() { return read('clientAudits', {}); }
function setClientAudit(clientId, { outcome, who, note }) {
  const all = getClientAudits();
  const valid = ['graduated', 'completed', 'free_round', 'in_progress'];
  if (outcome === null) {
    delete all[clientId];
    write('clientAudits', all);
    if (db.isEnabled()) db.query('delete from client_audits where ext_id = $1', [String(clientId)]).catch(e => console.error('Audit delete mirror failed:', e.message));
    return null;
  }
  if (!valid.includes(outcome)) throw new Error('outcome must be one of ' + valid.join(', '));
  const entry = { outcome, who: who || '', at: new Date().toISOString(), note: note || '' };
  all[clientId] = entry;
  write('clientAudits', all);
  if (db.isEnabled()) {
    db.query(
      `insert into client_audits (ext_id, outcome, audited_by, audited_at, note) values ($1,$2,$3,now(),$4)
       on conflict (ext_id) do update set outcome = excluded.outcome, audited_by = excluded.audited_by, audited_at = now(), note = excluded.note`,
      [String(clientId), outcome, entry.who, entry.note]
    ).catch(e => console.error('Audit mirror failed (still saved locally):', e.message));
  }
  return entry;
}
async function hydrateClientAuditsFromPostgres() {
  if (!db.isEnabled()) return;
  if (Object.keys(getClientAudits()).length) return;
  try {
    const { rows } = await db.query('select ext_id, outcome, audited_by, audited_at, note from client_audits');
    if (!rows.length) return;
    const all = {};
    for (const r of rows) all[r.ext_id] = {
      outcome: r.outcome, who: r.audited_by,
      at: r.audited_at instanceof Date ? r.audited_at.toISOString() : String(r.audited_at),
      note: r.note || ''
    };
    write('clientAudits', all);
    console.log(`Restored ${rows.length} client audits from Postgres`);
  } catch (e) { console.error('Client-audit hydration failed:', e.message); }
}
...
async function mirrorPersonalLayout(userId, layout) {
  ...
  try {
    ...
    await db.query(
      ...
      [pgId, JSON.stringify(layout && layout.nodes ? layout.nodes : []),
       // the widget_sizes column was an unused '{}' -- it now carries the
       // hidden-card list so hide/unhide survives a redeploy like positions do
       JSON.stringify({ hidden: (layout && layout.hidden) || [] })]
    );
  } catch (e) {
...
module.exports = {
  getReplyDismissals, setReplyDismissal, hydrateReplyDismissalsFromPostgres,
  getClientAudits, setClientAudit, hydrateClientAuditsFromPostgres,
