...
function throughput(entries, opts) {
  ...
  for (const e of entries || []) {
    ...
    const p = people[e.who] = people[e.who] || {
      who: e.who, total: 0, roundsFiled: 0, completed: 0, stageMoves: 0, messages: 0, notes: 0, docs: 0, other: 0,
      clients: new Set()
    };
    p.total++;
  ...
  const rows = Object.values(people).map(p => ({
    ...
    other: p.other, clientsTouched: p.clients.size
...
module.exports = { entriesFor, actionEntry, throughput, trim, classify, changed, MAX_ENTRIES };
