...
const EMPLOYEE_API = [
  ...
  // Clients access for the Team Dashboard's "Clients" nav item. mfsnCommission
  // (the one field still redacted -- see redactClient() below) stays out of
  // both routes regardless of this allowlist. Contact edit and the generic
  // tags route stay admin-only -- everything else on this record (status,
  // affiliate override, SMS, notes) is open as of 2026-08-05, explicitly
  // requested full parity with what Admin can do from the same drawer.
  { method: 'GET', pattern: /^\/api\/clients$/ },
  { method: 'GET', pattern: /^\/api\/clients\/[^/]+$/ },
  { method: 'POST', pattern: /^\/api\/clients\/[^/]+\/notes$/ },
  { method: 'POST', pattern: /^\/api\/clients\/[^/]+\/status$/ },
  { method: 'POST', pattern: /^\/api\/clients\/[^/]+\/affiliate$/ },
  { method: 'POST', pattern: /^\/api\/clients\/[^/]+\/sms$/ },
  // Moving a client to a different round -- the Pipeline board's "column"
  // for a GHL-sourced card -- explicitly open to both roles (2026-08-05),
  // same spirit as `stage` in EMPLOYEE_FIELDS above for a Deal Production
  // card. This is a narrow, dedicated tag swap (see the route itself), not
  // the generic /tags route, which stays admin-only.
  { method: 'POST', pattern: /^\/api\/clients\/[^/]+\/round$/ },
...
// appending notes. va (ownership reassignment) stays with the admin. cfpb
// (the per-round CFPB portal logins the team files complaints with)
// belongs here too -- it's the same day-to-day round work, not admin
// configuration.
//
...
// stages/columns from the Pipeline detail panel was explicitly opened to
// both roles (see POST /api/clients/:id/round for the GHL-round-based
// equivalent, opened the same day).
const EMPLOYEE_FIELDS = new Set(['tu', 'eq', 'ex', 'docs', 'note', 'cfpb', 'stage']);
...
// A client record mixes money in with the operational fields (name,
// contact, package, status, round, notes, MFSN affiliate status) an
// employee legitimately needs for the Clients table/drawer. Strip it here,
// server-side, rather than trusting the client to not render it -- same
// reasoning as EMPLOYEE_ASSETS above.
//
...
// Spent/# Pays to both roles now, and the drawer's Mark affiliate/not
// action needs the current status to render correctly. What's left is
// mfsnCommission -- a literal dollar figure -- which the Clients drawer
// deliberately never shows (its stat tiles for totalSpent/numberOfPayments
// and the Payment history list are gated client-side on `moneyVisible`
// instead, see openClient() in index.html): explicit request that day was
// "actions [Send text/Mark affiliate/Mark active/Add note], not numbers."
const EMPLOYEE_HIDDEN_CLIENT_FIELDS = new Set(['mfsnCommission']);
