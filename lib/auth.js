...
const CAPABILITIES = [
  ...
  'users',       // create/edit team accounts -- desk-manager work, NEVER admin ones (see /api/users guards)
...
const ROUTE_CAPS = [
  ...
  // Manual client creation -- Commas isn't wired to the app yet, so a new
  // sale has to be enterable by hand until that automation lands.
  { cap: 'production', method: 'POST', pattern: /^\/api\/production\/add$/ },
  // Mark MyFreeScoreNow standing from the production drawer or the dispute
  // desk -- both desks look at the same client, both may learn it changed.
  { cap: 'production', method: 'POST', pattern: /^\/api\/production\/[^/]+\/mfsn$/ },
  { cap: 'disputes', method: 'POST', pattern: /^\/api\/production\/[^/]+\/mfsn$/ },
  ...
  { cap: 'production', method: 'GET', pattern: /^\/api\/upsell$/ },
  { cap: 'production', method: 'GET', pattern: /^\/api\/ops$/ },
  // Everyone's numbers side by side is a management view, not desk work.
  { cap: 'admin', method: 'GET', pattern: /^\/api\/team-activity$/ },
  ...
  // The daily task checkbox ("I finished this client's round today") -- also
  // how an unassigned pick gets claimed, matching the team's random-pick flow.
  { cap: 'disputes', method: 'POST', pattern: /^\/api\/disputes\/[^/]+\/worked$/ },
  ...
  // Team-account management for desk managers (Nica): create members, edit
  // them, resend setup links. The routes themselves refuse anything touching
  // an admin account when the caller isn't one -- capability opens the door,
  // the escalation guards decide what can walk through it.
  { cap: 'users', method: 'POST', pattern: /^\/api\/users$/ },
  { cap: 'users', method: 'PATCH', pattern: /^\/api\/users\/[^/]+$/ },
  { cap: 'users', method: 'POST', pattern: /^\/api\/users\/[^/]+\/invite$/ },
  ...
  // Marking a thread unread (or read) is inbox triage, the VA's actual job.
  { cap: 'messages', method: 'POST', pattern: /^\/api\/messages\/[^/]+\/unread$/ },
  { cap: 'messages', method: 'POST', pattern: /^\/api\/messages\/[^/]+\/no-reply$/ },
  { cap: 'messages', method: 'POST', pattern: /^\/api\/messages\/[^/]+\/attachments$/ },
  { cap: 'messages', method: 'GET', pattern: /^\/api\/snippets$/ },
  { cap: 'messages', method: 'POST', pattern: /^\/api\/snippets$/ },
  { cap: 'messages', method: 'DELETE', pattern: /^\/api\/snippets\/[^/]+$/ },
...
const COMMON_ASSETS = new Set([
  '/', '/index.html', '/role.js', '/team.js', '/logo.png', '/logo-mark.png',
  '/mfsn-logo.png', '/login.html', '/set-password.html', '/favicon.ico'
]);
...
// `pkg` is here because the package decides the round allowance, and the
// allowance was being read from a name nobody could correct when it was wrong.
// 49 clients still carry a package the parser cannot read; without this the
// only fix was editing the sheet the records were seeded from.
const PRODUCTION_FIELDS = new Set(['tu', 'eq', 'ex', 'docs', 'note', 'cfpb', 'stage', 'pkg']);
...
// CFPB portal logins and passwords. Filing the complaint is the disputer's
// job, so nobody else has a reason to hold them -- and until now every session
// with `production` was sent all of them: 8,285 real passwords across 3,259
// clients, on every Deal Production page load, including VAs, who cannot even
// open the dispute desk.
//
// They are encrypted at rest, which is worth nothing if they are decrypted and
// broadcast to the browser. A list view never needs a password; only the
// per-client record does.
function stripCfpbSecrets(actor, record) {
  if (!record || !Array.isArray(record.cfpb) || !record.cfpb.length) return record;
  if (capsFor(actor).has('disputes')) return record;
  return Object.assign({}, record, {
    // The round and the filing date are ordinary progress information. It is
    // the credentials that come out.
    cfpb: record.cfpb.map(e => ({ round: e && e.round, date: e && e.date }))
  });
}
// The LIST always drops them, for everyone including an admin. Least privilege
// belongs to the surface as much as the role: a table of 3,885 clients has no
// use for a password, so there is no reason for 8,285 of them to cross the
// wire and sit in browser memory. The per-client record is where they belong,
// gated by stripCfpbSecrets above.
function stripCfpbSecretsAll(records) {
  if (!Array.isArray(records)) return records;
  return records.map(r => {
    if (!r || !Array.isArray(r.cfpb) || !r.cfpb.length) return r;
    return Object.assign({}, r, { cfpb: r.cfpb.map(e => ({ round: e && e.round, date: e && e.date })) });
  });
}
...
// Every dollar-bearing field a client record carries. Stripped server-side
// for any session without the revenue capability -- hiding a column in the UI
// is not a boundary, the payload is. Payment DATES stay: they drive the
// onboarding and never-started clocks, which are the VA's actual work, and a
// date is when something happened, not what it cost.
const MONEY_CLIENT_FIELDS = new Set(['mfsnCommission', 'totalSpent', 'numberOfPayments', 'paymentCount']);
...
module.exports = {
  ...
  stripCfpbSecrets, stripCfpbSecretsAll,
  EMPLOYEE_FIELDS
};
