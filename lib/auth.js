...
const SELF_API = [
  ...
  // The Today queue is deliberately universal. It is not "an admin page with
  // a disputer mode" -- buildQueue() never constructs the money-bearing items
  // for a caller without the revenue capability, so there is nothing here to
  // gate that isn't already gated at the source.
  { method: 'GET', pattern: /^\/api\/actions$/ },
  { method: 'POST', pattern: /^\/api\/actions\/done$/ },
...
const COMMON_ASSETS = new Set([
  ...
  // actions.js renders the Today queue for every role. It is common rather
  // than capability-gated because the page shows whatever /api/actions
  // returns, and that endpoint never builds money-bearing items for a caller
  // without the revenue capability -- see lib/actions.js buildQueue().
  '/actions.js'
]);
...
module.exports = {
  ...
  PRODUCTION_FIELDS, DISPUTE_FIELDS,
  EMPLOYEE_FIELDS
};
