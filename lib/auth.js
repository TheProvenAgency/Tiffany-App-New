...
const EMPLOYEE_API = [
  ...
  // Follow-Ups (the /api/tasks family) used to be open to both roles --
  // deliberately NOT anymore. The 2026-08-05 Company vs Team Dashboard spec
  // dropped Follow-Ups from the Employee nav entirely and asked for it to be
  // "blocked at the route level," not just hidden, so there is no longer an
  // employee-reachable entry here at all (the Clients-drawer "+ Follow-up
  // task" control and Add-note section are hidden client-side to match --
  // see the .employeeHide class in index.html's openClient()).
