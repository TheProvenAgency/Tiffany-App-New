...
const EMPLOYEE_API = [
  ...
  // (already-redacted) route is reachable at all. Every OTHER write on a
  // client (status, affiliate override, contact edit, tags, SMS) stays
  // admin-only on purpose.
  { method: 'GET', pattern: /^\/api\/clients$/ },
  { method: 'GET', pattern: /^\/api\/clients\/[^/]+$/ },
  // Adding a note is explicitly open to both roles (2026-08-05) -- routine
  // client contact from the Pipeline/Clients detail panel, same spirit as
  // Deal Production's own note field.
