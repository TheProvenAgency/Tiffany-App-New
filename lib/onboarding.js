...
function toRow(c, now, slaDays) {
  // The real first purchase when we have it (lib/purchases.js), falling back
  // to whatever last-payment stand-in exists. Measuring from the LAST payment
  // understates every wait: a client who bought in January and paid again in
  // April reads as an April arrival.
  const from = c.firstPaid || c.lastPaid;
