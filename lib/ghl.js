...
async function sendMessage(cfg, { contactId, conversationId, type = 'SMS', message, subject, attachments } = {}) {
...
// Upload a file to GHL's message-attachment store. Multipart, not JSON --
// GHL hosts the file and returns URL(s) that sendMessage() can then carry in
// its attachments array, exactly what GHL's own composer does under the
// hood. Content-Type is left to FormData so the multipart boundary is right.
async function uploadMessageAttachment(cfg, { conversationId, filename, buffer, mime }) {
  const fd = new FormData();
  fd.append('conversationId', conversationId);
  fd.append('locationId', cfg.ghlLocationId);
  fd.append('fileAttachment', new Blob([buffer], { type: mime || 'application/octet-stream' }), filename || 'file');
  const h = headers(cfg); delete h['Content-Type'];
  const res = await fetch(BASE + '/conversations/messages/upload', { method: 'POST', headers: h, body: fd });
  if (!res.ok) {
    const detail = await res.text().catch(() => '');
    const err = new Error('GoHighLevel refused the upload (' + res.status + ')');
    err.detail = detail.slice(0, 500);
    throw err;
  }
  const data = await res.json();
  // Response shape has varied across API versions; accept the known ones.
  const urls = data.uploadedFiles ? Object.values(data.uploadedFiles)
    : data.urls || (data.url ? [data.url] : []);
  return urls.flat().filter(Boolean);
}

// The sub-account's snippets/templates, for the composer's snippet picker.
// GHL keeps them under locations/{id}/templates; body fields differ between
// SMS and email templates, so normalize both to {id, name, body}.
// Pages until GHL runs out ("a lot of snippets aren't showing" was this
// fetch stopping at the first 100), and accepts every body shape GHL uses
// across SMS templates, email templates, and snippet variants -- a snippet
// whose body sits in a field this list doesn't know is kept with its name
// visible rather than silently dropped, so a gap is at least SEEN.
async function fetchSnippets(cfg) {
  const out = [];
  for (let skip = 0; skip < 1000; skip += 100) {
    const data = await ghlFetch(cfg, `/locations/${cfg.ghlLocationId}/templates?deleted=false&skip=${skip}&limit=100&originId=${cfg.ghlLocationId}`);
  ...
  return out.map(t => {
    ...
    const html = tpl.html || t.html || '';
    const body = tpl.body || tpl.message || tpl.text || t.body || t.message || t.snippet
...
module.exports = {
  ...
  uploadMessageAttachment, fetchSnippets,
