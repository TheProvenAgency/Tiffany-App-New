...
async function fetchSnippets(cfg) {
  const data = await ghlFetch(cfg, `/locations/${cfg.ghlLocationId}/templates?deleted=false&skip=0&limit=100&originId=${cfg.ghlLocationId}`);
  const list = data.templates || data.data || [];
  return list.map(t => ({
    id: t.id,
    name: t.name || t.title || 'Untitled',
    type: (t.type || '').toLowerCase(),
    body: (t.template && (t.template.body || t.template.message || t.template.html))
      || t.body || t.message || ''
  })).filter(t => t.body);
}
