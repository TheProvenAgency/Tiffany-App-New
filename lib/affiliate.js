...
// her own affiliate link ("affiliate"), on MyFreeScoreNow but NOT under her
// link ("not_affiliate" -- an active MFSN user, just not one she's getting
// credit for), or not found on MyFreeScoreNow at all ("not_on_mfsn").
// Members arrive from her "Active List" export (manually re-synced today;
// eventually a scheduled Zapier "Fetch Active Members List" Zap); clients
// come from the roster (GHL contacts, or the Deal Production tracker).
//
...
//
// Whether a matched member counts as "affiliate" vs "not_affiliate" comes
// down to the enrollment link they signed up through (the "Upgrade Link"
// column in her Active List export, e.g.
// https://app.myfreescorenow.com/enroll/B01B1514?...) -- only the codes in
// AFFILIATE_CODES are hers; everything else (a different code, or no code
// captured at all) means they're on MyFreeScoreNow but not enrolled under
// her, which is exactly the group worth reaching out to.

// Confirmed with Tiffany 2026-07-28: these three codes are hers (a
// primary link plus two secondary/campaign links). Anything else in the
// Upgrade Link column -- a different code, or none at all -- does NOT
// count as her affiliate.
...
function normalizeMembers(members) {
  ...
  for (const m of members || []) {
    ...
    if (existing) existing.hasAffiliateCode = existing.hasAffiliateCode || hasAffiliateCode;
    else seen.set(key, { email, name, hasAffiliateCode });
  }
...
function affiliateGap(clients, enrolled, overrides) {
  ...
  const tagged = (clients || []).map(c => {
    ...
    const computed = !matched ? 'not_on_mfsn' : (member.hasAffiliateCode ? 'affiliate' : 'not_affiliate');
    const override = overrides[c.id] || null;
    const mfsnStatus = override || computed;
    return { ...c, mfsnStatus, mfsnMatched: matched, mfsnOverride: override };
  });
