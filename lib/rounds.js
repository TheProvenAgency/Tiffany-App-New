...
const ADDON = 'addon';     // one targeted job, not a course of rounds
const REFUND = 'refunded'; // not a client at all

// Packages whose names carry no number but whose allowance is known.
const NAMED = {
  'help me fix it': 1,
  // (HLP) is the abbreviation for "Help me fix it" -- 31 clients, and the
  // priced version of the same package is $50, which is exactly one round at
  // the $50-a-round rate every readable package in the book charges.
  '(hlp)': 1,
  'hlp': 1,
  'quick-fix': 1,
...
// Targeted one-off work rather than a course of rounds: a bureau sweep, an
// expedited removal, an inquiry removal, a tradeline, funding. 72 clients hold
// one of these, and 68 of them hold nothing else -- so calling their round
// allowance "unknown" was wrong twice over. It is not unknown, there simply
// isn't one: the job is the job, and it is done when the pipeline says so.
const ADDON_WORK = /sweep|removal|inquiry|tradeline|funding|credit card|late payment|backdoor|nfcu/i;

// A refunded sale is not a client to upsell. Two records carry this as their
// package and would otherwise sit in the finished list.
const REFUNDED = /^\s*refunded\s*$/i;
...
function roundsIncluded(pkg) {
  ...
  // the original number.
  if (REFUNDED.test(raw)) return REFUND;
  if (/unlimited/.test(raw)) return UNLIMITED;
  if (OUTCOME_BASED.test(raw)) return OUTCOME;
  // Only when the whole package is add-on work. Someone who bought a sweep
  // ALONGSIDE a round package still has that package's allowance, so this must
  // not short-circuit a name that also carries a count.
  if (ADDON_WORK.test(raw) && !/(\d+\s*[-\s]?\s*(?:[a-z]+\s+)?(month|round))|full|unlimited|diamond/i.test(raw)) return ADDON;
  ...
  // Add-ons alone sum to zero, which is not "bought no rounds" -- it is
  // "bought no round package". ADDON above already covers the whole-string
  // case; this catches a string of several add-ons.
  if (sawNumber && total === 0) return ADDON;
...
function segmentRounds(raw) {
  if (!raw) return null;
  // An add-on segment contributes no rounds -- but that is a known zero, not
  // an unreadable one, so "Experian Removal, 3 Month Expedited" is exactly 3
  // rather than "at least 3". Without this the pairing reads as uncertain and
  // never qualifies as finished.
  if (ADDON_WORK.test(raw) && !/\d+\s*[-\s]?\s*(?:[a-z]+\s+)?(month|round)/.test(raw)) return 0;
...
function forClient(client) {
  ...
  if (inc === REFUND) {
    return { roundsIncluded: REFUND, allowanceExact: true, roundsUsed: used,
             roundsLeft: null, finished: false, finishedBy: null };
  }
  if (inc === UNLIMITED || inc === OUTCOME || inc === ADDON) {
    return {
...
function upsellQueue(clients, opts) {
  ...
  return {
    ...
    totals: {
      ...
      addOnOnly: all.filter(c => c.roundsIncluded === ADDON).length,
      refunded: all.filter(c => c.roundsIncluded === REFUND).length,
...
module.exports = { roundsIncluded, segmentRounds, roundsUsed, forClient, attach, upsellQueue, UNLIMITED, OUTCOME, ADDON, REFUND };
